...
grid on
title('Penerapan Metode RKF45 Orde 5 Untuk Titik Ekuilibrium Endemik')
...
%Error Relatif
for i=1:N
    ES(i)=abs(xsol((i),1)-S(i))/abs(xsol((i),1));
    EE(i)=abs(xsol((i),2)-E(i))/abs(xsol((i),2));
    EI(i)=abs(xsol((i),3)-I(i))/abs(xsol((i),3));
    ER(i)=abs(xsol((i),3)-I(i))/abs(xsol((i),4));
end
